...
2.  **Level Boundaries**: The crucial part is `level_size = len(queue)`. By capturing the size at the start of the `while` loop, we know exactly how many nodes belong to the *current* level.
3.  **Iteration**: We pop exactly `level_size` nodes, add their values to a list, and push their children into the queue. These children will form the *next* level.
...
3.  **Consistency**: The nodes are still added to the main `queue` in the same order (left then right). Only the *storage* of their values in the `current_level` list changes.
