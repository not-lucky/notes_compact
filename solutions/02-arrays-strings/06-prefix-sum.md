...
**Explanation**:
Similar to Problem 4, we use remainders. To ensure the length is at least 2, we store the *index* of the first time we saw a remainder. If we see the same remainder at index `i`, and `i - first_index >= 2`, we found a valid subarray.
...
**Explanation**:
We use the prefix sum approach with a hash map. Since we want the *maximum* length, we only record the index of the *first* time we see a particular prefix sum. This ensures that when we find a match, the distance between the current index and the recorded index is as large as possible.
