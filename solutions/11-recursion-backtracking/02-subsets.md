...
2. **Duplicate Skipping**: Inside the loop, `if i > start and nums[i] == nums[i-1]` checks if the current element is a duplicate of the previous one *at the same level of the recursion tree*. If we just popped a `2` and the next element is also `2`, we skip it to avoid generating the same subset again.
