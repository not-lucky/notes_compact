...
2.  **Searching**: We traverse the string a second time. Since we want the *first* unique character, we must iterate through the string characters in their original order. For each character, we check our frequency map. The first character with a count of 1 is our answer.
