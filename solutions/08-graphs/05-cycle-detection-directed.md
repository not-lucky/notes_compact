...
**Difficulty:** Hard
**Concept:** Find edge causing cycle in directed graph
...
def find_redundant_directed_connection(edges: List[List[int]]) -> List[int]:
    """
    Find the edge that makes the graph not a rooted tree.
    Three cases:
    1. A node has two parents.
    2. There is a cycle.
    3. Both (a node has two parents and there is a cycle).
