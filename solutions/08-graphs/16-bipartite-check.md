...
def can_color(n: int, edges: List[List[int]], k: int) -> bool:
    """
    Check if graph can be colored with k colors such that no two adjacent
    nodes have the same color. (NP-Complete for k >= 3, using backtracking).
