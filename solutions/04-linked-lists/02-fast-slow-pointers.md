...
### Explanation
We use a `dummy` node to simplify the case where the head is removed. By moving `first` pointer `n+1` steps ahead, we establish a gap. When `first` hits `None`, `second` is exactly at the node *preceding* the target. We then just skip the target node.
