...
2.  **Variables**:
    ...
    - `current_end`: The boundary of the current jump. Once we cross this, we *must* have made another jump.
3.  **Greedy Step**: We don't care exactly *which* index we jump to; we just know that for a given jump, we want to pick the next jump point that gives us the maximum future reach.
