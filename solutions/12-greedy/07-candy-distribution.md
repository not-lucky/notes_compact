...
- **Example 2**:
    - Input: `ratings = [1,2,2]`
    - Output: `4`
    - Explanation: You can allocate to the first, second and third child with 1, 2, 1 candies respectively. The third child gets 1 candy because it satisfies the above two conditions (it doesn't have a higher rating than its left neighbor, so it doesn't *need* more).
- **Edge Cases**:
...
4.  **Local Optimality**: At each step, we provide the *minimum* possible candy to satisfy the immediate constraint, which leads to a globally minimal sum.
...
3.  **Why it works**: Even if `first` is updated *after* `second` was set (meaning the actual `first` that made `second` valid is further left), the fact that we found something larger than `second` still guarantees a triplet exists.
