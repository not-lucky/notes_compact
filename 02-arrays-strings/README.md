...
**Mental Models**:
- **Array**: Think of an array as a row of connected, numbered mailboxes. You can instantly access any mailbox if you know its number ($\Theta(1)$ time). But if you need to insert a new mailbox in the middle, you have to physically shift all the subsequent mailboxes down to make room ($\Theta(n)$ time).
- **String**: Think of a string as a necklace of beads. You can inspect the sequence of beads easily, but because the necklace is tied (immutable in Python), to change a single bead, you have to unstring and re-string a completely new necklace ($\Theta(n)$ time).
...
1. **Lists are Dynamic Arrays**: A Python `list` is a dynamic array, not a linked list. Appending an element (`list.append()`) is **amortized $\Theta(1)$** time. However, inserting or deleting from the middle (`list.insert(0, val)`) takes $\Theta(n)$ time because all subsequent elements must be shifted in memory.
2. **String Concatenation**: Python strings are immutable. Building a string in a loop using `+=` forces Python to allocate new memory and copy the old characters each time. This results in **$\Theta(n^2)$ time**. Always use `''.join(list_of_strings)` for linear **$\Theta(n)$** concatenation.
3. **Hash Maps (Dictionaries)**: Lookups, insertions, and deletions in Python `dict`s and `set`s are **amortized $\Theta(1)$** time on average. However, it's crucial to mention to your interviewer that the **worst-case time complexity is $\Theta(n)$** due to potential hash collisions.
4. **Tight Bounds ($\Theta$)**: Always provide the tightest bound possible. If an algorithm always iterates through exactly $n$ elements, say $\Theta(n)$, not $O(n^2)$, even though mathematically $O(n^2)$ is technically an upper bound. While interviewers casually say "Big-O", they almost always mean "Big-Theta" ($\Theta$) tight bounds.
5. **Recursive Space Complexity**: If you use recursion for any array or string traversal, you **must** account for the recursive call stack in your space complexity analysis. An algorithm that recurses $n$ times has a $\Theta(n)$ space complexity footprint, even if it uses no extra variables.
...
| "Rearrange elements in-place with $\Theta(1)$ space" | **Two Pointers** (same direction / swap logic) |
...
| [06-prefix-sum](./06-prefix-sum.md)                                   | Prefix Sum                        | $\Theta(1)$ range queries after $\Theta(n)$ setup |
| [07-difference-array](./07-difference-array.md)                       | Difference Array                  | Efficient overlapping range updates         |
| [08-kadanes-algorithm](./08-kadanes-algorithm.md)                     | Kadane's Algorithm                | Maximum subarray sum in $\Theta(n)$         |
...
| [14-in-place-modifications](./14-in-place-modifications.md)           | In-Place Modifications            | $\Theta(1)$ space tricks, indexing as state |
...
2. **String immutability confusion**: Using `+=` instead of `.join()`. Interviewers will dock points for saying string concatenation in a loop is $\Theta(n)$. It is $\Theta(n^2)$.
3. **Ignoring hash map worst-case**: Failing to mention that while hash map lookups are amortized $\Theta(1)$, worst-case collisions can degrade to $\Theta(n)$.
...
def two_sum(nums: list[int], target: int) -> list[int]:
    ...
    Time Complexity: Θ(n) tightly bound, as we iterate at most n times.
    Space Complexity: Θ(n) for the hash map storing seen values.
    """
    seen: dict[int, int] = {}

    for i, num in enumerate(nums):
        complement = target - num
        # Hash map lookup is amortized Θ(1), worst-case Θ(n)
...
Understanding Big-O and Big-Theta ($\Theta$) is essential before this chapter. Ensure you are comfortable distinguishing between amortized and worst-case complexities, recognizing recursive call stack overhead, and calculating tight bounds ($\Theta$).
