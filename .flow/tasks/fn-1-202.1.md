# fn-1-202.1 Map and Audit Existing Diagrams

## Description
Scan all `.md` files in the repository to identify and map all existing diagrams. Document the specific type of diagram (flowchart, data structure, etc.) and identify clear cases of "off by 1" misalignment or use of legacy ASCII characters (e.g., using `+`, `-`, `|` instead of Unicode).

**Size:** S
**Files:** All `.md` files

## Approach
- Use `grep` and `find` to locate code blocks.
- Create a temporary tracking list of files that need updates.
- Note specific line numbers for misalignment.
## Acceptance
- [ ] `survey_results.md` (or similar) created with a list of files and diagram types.
- [ ] Clear understanding of which diagrams are broken visually.
## Done summary
