...
## 0. Warm-Up: Simple String Matching with `.` (No `*`)

Before tackling the complexities of `*`, let's understand how basic string matching works when we only have literal characters and a single-character wildcard (`.` or `?`). This sets up the foundation for our DP approach.

**Problem:** Given a string `s` and a pattern `p`, check if they match. `p` can contain `.` which matches any single character.

**Approach:** We can solve this with a simple two-pointer or recursive approach. If the characters match (or if the pattern has a `.`), we advance both pointers.

### Recursive Approach (Top-Down intuition)

```python
def is_match_simple(s: str, p: str) -> bool:
    ...
    # Recurse on the rest of the string and pattern
    return first_match and is_match_simple(s[1:], p[1:])
```

Notice how `is_match_simple` evaluates character by character. When we introduce `*`, we can't simply move both pointers forward by 1; we have to *branch* because `*` can mean different things. That branching creates overlapping subproblems, which is why we need memoization or tabulation.
...
Let `dp[i][j]` be `True` if the suffix `s[i:]` matches the suffix `p[j:]` (for Top-Down) or if the prefix `s[:i]` matches the prefix `p[:j]` (for Bottom-Up).
...
def is_match_wildcard_memo(s: str, p: str) -> bool:
    ...
    def dfs(i: int, j: int) -> bool:
        ...
        if p[j] == '*':
            ...
            # Note: For Branch 2 to be valid, we don't strictly require `match` to be true for * here, 
            # because * matches ANY sequence, we just need i < len(s) to consume a character.
...
Similar to regex matching, wildcard matching relies only on the current and previous rows, so it can be optimized to $O(n)$ space.
...
1. **[Longest Common Subsequence (LCS)](https://leetcode.com/problems/longest-common-subsequence/)**: Master the basic `dp[i][j]` grid comparing characters.
