...
- **Big-O/Theta Precision:** Use $\Theta$ when bounds are tight and known exactly (like traversal $\Theta(n)$), and $O$ for worst-case bounds. Ensure tables and inline text format them cleanly (e.g., `Time Complexity: $\Theta(n)$`).- **Stacks & Queues (Chapter 5):** When generating valid parentheses, specify that the time complexity is bounded by the Catalan number $\mathcal{O}(4^N / \sqrt{N})$ rather than just stating "Catalan number". Ensure that even simple helper functions without docstrings get standard complexity analysis added (e.g., `is_valid_simple` counting balance).
