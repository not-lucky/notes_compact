...
- **Tree Serialization & String Parsing:** When parsing tree structure out of strings (like parentheses notation `1(2)(3)`), avoid $\mathcal{O}(N^2)$ string slicing inside recursion (e.g. `data[start:end]`). Instead, use a single shared mutable index pointer (e.g. `i = [0]`) and sequentially parse the string in $\mathcal{O}(N)$ time.
- **BST Deserialization from Preorder:** When dynamically reconstructing a BST from its preorder sequence in $\mathcal{O}(N)$ time, rely on strict `min_val` and `max_val` parameter constraints. If the current sequence value violates the bounds, return `None` rather than trying to search or slice.
