...
- **Tuples inside `heapq`:** Python heaps compare tuples element by element. When adding nodes to a heap, always include an `index` as a tiebreaker (e.g., `(val, index, node)`) to prevent Python from attempting (and failing) to compare the `ListNode` objects directly when values are equal.
